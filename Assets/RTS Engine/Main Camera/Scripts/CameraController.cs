...
namespace RTSCamera
{
    public class CameraController : MonoBehaviour
    {
        ...
        {
            Vector3 nextPosition = new Vector3(targetPosition.x + currOffsetX, mainCamera.transform.position.y, targetPosition.z + currOffsetZ);
            //if the next position (after applying the offsets is not inside the panning limits)
            if (panLimit.enabled
                && (nextPosition.x < panLimit.minPosition.x || nextPosition.y < panLimit.minPosition.y || nextPosition.x > panLimit.maxPosition.x || nextPosition.y > panLimit.maxPosition.y))
                return;
