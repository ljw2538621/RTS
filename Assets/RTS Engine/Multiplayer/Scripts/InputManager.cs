    ...
    public class InputManager : MonoBehaviour
    {
        ...
        public void OnUnitGroupMovementCommand(NetworkInput command)
        {
            ...
            if (unitList.Count > 0) //if there's actual units in the list
            {
                if (command.targetMode == (byte)InputMode.attack) //if the target mode is attack -> make the unit group launch an attack on the target.
                {
                    ...
                    gameMgr.MvtMgr.PrepareMove(unitList, command.targetPosition, targetEntity.GetRadius(), targetEntity, InputMode.attack, command.playerCommand, true, (MovementManager.AttackModes)command.value, targetEntity);
                }
                else //target movement type can be none, portal, APC, etc...
                    gameMgr.MvtMgr.PrepareMove(unitList, command.targetPosition, command.value, null, (InputMode)command.targetMode, command.playerCommand);
            }
        }
        ...
        public void OnUnitCommand(NetworkInput command)
        {
            ...
            if (command.targetMode == (byte)InputMode.movement) //no target mode -> movement
                gameMgr.MvtMgr.PrepareMove(sourceUnit, command.targetPosition, command.value, null, InputMode.movement, command.playerCommand);
            else if (spawnedObjects.TryGetValue(command.targetID, out Entity target))
            {
                switch ((InputMode)command.targetMode)
                {
                    case InputMode.APC: //targetting a APC
                        ...
                        //move to target APC
                        gameMgr.MvtMgr.PrepareMove(sourceUnit, command.targetPosition, command.value, target, InputMode.APC, command.playerCommand);
                        break;
                    ...
                    case InputMode.portal: //moving to a portal
                        ...
                        gameMgr.MvtMgr.PrepareMove(sourceUnit, command.targetPosition, command.value, target, InputMode.portal, command.playerCommand);
                        break;
                    ...
                    case InputMode.attack: //attacking the target object
                        ...
                        gameMgr.MvtMgr.PrepareMove(sourceUnit, command.targetPosition, targetEntity.GetRadius(), targetEntity, InputMode.attack, command.playerCommand, true, (MovementManager.AttackModes)command.value, targetEntity);
                        break;
