...
namespace RTSEngine
{
    ...
    public class NPCBuildingCreator : NPCComponent
    {
        ...
        {
            NPCBuildingRegulatorData data = building.GetRegulatorData(factionMgr.Slot.GetTypeInfo(), npcMgr.NPCType.GetCode()); //get the regulator data
        ...
        protected override void OnActiveUpdate()
        {
            ...
            foreach (BuildingCenterRegulator bcr in buildingCenterRegulators)
                foreach (ActiveBuildingRegulator abr in bcr.activeBuildingRegulators.Values)
                {
                    ...
                    if (!abr.instance.HasReachedMinAmount() && !abr.instance.HasReachedMaxAmount())
                    {
