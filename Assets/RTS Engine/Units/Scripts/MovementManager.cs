...
/* MovementManager created by Oussama Bouanani, SoumiDelRio.
...
namespace RTSEngine
{
    public class MovementManager : MonoBehaviour
    {
        ...
        public struct MovementFormation
        {
            ...
            //for the rectangular movement formation:
            [Header("Rectangular Formation"), Tooltip("Maximum amount of units per row in a rectangular formation.")]
            public int unitsPerRow;
            [Tooltip("Maximum amount of allowed empty rows before switching back to a circular formation.")]
        ...
        {
            return (attackMovement == true) ? unit.AttackComp.GetRange().GetAttackFormation() : ((unit.AttackComp != null) ? movementFormation.attackUnits : movementFormation.nonAttackUnits);
        }
        ...
        private ChainedSortedList<float, Unit> SortUnits(List<Unit> units, UnitSortTypes sortType, float offsetRadius, AttackModes attackMode = AttackModes.none, FactionEntity targetEntity = null, Vector3 targetPosition = default, bool playerCommand = false)
        {
            ...
            foreach (Unit unit in units) //go through the list of units.
            {
                ...
                if (sortType != UnitSortTypes.radius) //if we're not sorting by radius <=> we're not sorting for a simple movement but for an attack movement and therefore:
                {
                    ...
                    sortKey = unit.AttackComp.GetRange().GetStoppingDistance(targetEntity); //update sort key
        ...
        public List<Vector3> GenerateTargetPositions(Formations formation, Vector3 originPosition, int amount, float unitRadius, LayerMask agentAreaMask, bool isFlyUnit, ref float offsetRadius, Vector3 unitsDirection)
        {
            ...
            int emptyRowsCount = 0; //holds the amount of empty rows in a rectangular formation:
            ...
            Vector3 possiblePosition = Vector3.zero; //where each potential target position will be stored to be tested whether it is valid or not
                ...
                switch (formation)
                {
                    case Formations.circular: //in case we have a circular formation
                        ...
                        while (subCounter < expectedPositionCount) //as long as we haven't inspected all the expected free positions inside this cirlce
                        {
                            ...
                            possiblePosition = new Vector3(x, gameMgr.TerrainMgr.SampleHeight(new Vector3(x, originPosition.y, z), unitRadius, agentAreaMask), z);
                    ...
                    case Formations.rectangular:
                        ...
                        Vector3 rowDirection = new Vector3(unitsDirection.z, unitsDirection.y, -unitsDirection.x).normalized; //get the row's direction from the input unitsDirection

                        possiblePosition = originPosition; //start with the origin position (destination) as a possible position
                        //every time "move back" the size of the unit's radius multiplied by the row size to have a space to get a new row
                        possiblePosition -= new Vector3((unitRadius * rowSize) * rowDirection.x, 0.0f, (unitRadius * rowSize) * rowDirection.z);

                        int lastTargetPositionsCount = targetPositions.Count; //save the current target positions count before adding new target positions to it

                        subCounter = 0; //counter that helps with checking all positions in next row
                        while (subCounter < rowSize)
                        {
                            //sample height to get the positiosample height to get the position on the y axis for the next potential target position
                            possiblePosition.y = gameMgr.TerrainMgr.SampleHeight(possiblePosition, unitRadius, agentAreaMask);
                            //test whether the calculate target position is valid or not
                            if (IsPositionClear(ref possiblePosition, unitRadius, agentAreaMask, isFlyUnit))
                            {
                                targetPositions.Add(possiblePosition);
                                originPosition.y = possiblePosition.y;
                            }

                            //calculate the next potential target position to test by moving the size of the unit radius in the row's direction
                            possiblePosition += new Vector3((unitRadius * 2.0f) * rowDirection.x, 0.0f, (unitRadius * 2.0f) * rowDirection.z);

                            subCounter++;
                        }

                        //reset the origin position so we can calculate the next row in the next iteration (use unitsDirection to get the origin position in the row behind the current one)
                        originPosition -= new Vector3((unitRadius * 2.0f) * unitsDirection.x, 0.0f, (unitRadius * 2.0f) * unitsDirection.z);

                        //check whether the last row was empty or not and update counters accordingly:
                        if (lastTargetPositionsCount == targetPositions.Count)
                            emptyRowsCount++;

                        if (emptyRowsCount >= movementFormation.maxEmptyRows //if the maximum allowed amount of empty rows has been reached
                            || mainCounter <= 0 && targetPositions.Count < amount) //or if we haven't got the target amount we require and had at least as many attempts as the target amount
                        {
                            formation = Formations.circular; //something might be blocking the current direction of the rectangular formation so we move to the circular one to avoid this
                            mainCounter = targetPositionGenerationMaxIterations + amount; //counter for the while loop
                        }
        ...
        public bool CanProceedAttackMovement (Unit unit, bool playerCommand, bool assignedAttackOnly, bool canChangeTarget, FactionEntity targetEntity)
        {
            ...
            {
                if (playerCommand) //player notification message
